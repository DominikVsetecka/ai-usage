...
- `2026-06-24` — Reset-Countdown im Popover: `ProviderUsageWindow` parst `resetDescription` (z.B. „Resets 1:30am (Europe/Vienna)") automatisch zu `resetsAt: Date?`. `WindowRow` zeigt „Resets in X hr Y min" berechnet aus `resetsAt - Date.now`; fällt auf den Raw-Text zurück wenn kein Datum parsebar. Absolute Zeit-Formate (time-only, month+day+time, relative) und Zeitzone aus Klammern werden unterstützt.
- `2026-06-24` — Claude reset-Zeit Fix (final): SwiftTerm als Dependency hinzugefügt; `TerminalRenderer` (SwiftTerm-basiert) ersetzt den bisherigen Regex-ANSI-Stripper komplett. SwiftTerm simuliert einen echten VT100-Screen-Buffer und rekonstruiert den sichtbaren Text korrekt — inkl. Cursor-Bewegungen, Screen-Clears, Partial-Line-Overwrites. Ergebnis: `5h reset=Resets 1:30am (Europe/Vienna)` und `1w reset=Resets Jun 30 at 9pm (Europe/Vienna)` werden korrekt und getrennt erkannt.
